...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  Serial.print(topic);
  Serial.print("] ");
  char *payloadCharArr = new char[length];
  //  c++ new char arr
  // https://blog.csdn.net/weixin_27729671/article/details/113537162
  ...
  //  char *payloadCharArr=new char [1111];
  //  byte * 转化为 String
  display.clear();
  display.drawString(0, 0, String(topic));
  //  byte *
  // display.drawString(20, 0, String(payloadCharArr));
  // 这是同一行的
  display.drawString(0, 40, String(payloadCharArr));
  // call of overloaded 'String(byte*&)' is ambiguous
  //
  display.display();
